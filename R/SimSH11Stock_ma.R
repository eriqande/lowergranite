#' Run the steelhead script
#' 
#' This is kirk's script that eric wrapped up in a function
...
#' @param DO_GSI_ON_PTABLE if set to TRUE then the script simulates gsi_sim assignments from the tabulated counts of fish from different
#' stocks in each stratum. If this and \code{DO_GSI_ON_PROP} are both FALSE, it uses the true origins.
#' @param DO_GSI_ON_PROP if set to TRUE then gsi_sim is used to create assignments that replace the assignments in the variable Prop.  This is 
#' required if doing stock-sex or stock-age combinations.
#' @param alph One minus the size of the desired confidence interval to be calculated
#' @param B number of bootstrap replicates
#' @param nsim number of simulation replicates to do
#' @export
run_steelhead_script <- function(
	DAT.DIR = system.file("data_files", package="lowergranite", mustWork=T),  
  WORK.DIR = getwd(),  
  STOCK.DATA.XLSX = file.path(DAT.DIR, "SH11SIMPOPstock.xlsx"),
	DO_GSI_ON_PTABLE  = TRUE,
  ...
  GSISIM.FOLDER = getwd(),          # path to the gsi_sim analysis folder
  alph = 0.10,
  B = 500,
  nsim = 500
) {
  ...
  if(DO_GSI_ON_PTABLE && DO_GSI_ON_PROP) stop("Only one of DO_GSI_SIM_ON_PROP or DO_GSI_SIM_ON_PTABLE can be TRUE")
  DO_GSI <- xor(DO_GSI_ON_PTABLE, DO_GSI_ON_PROP)   # this will do GSI stuff that must be done for either GSI method
  ...
  # Kluge the Ptable because not all groups occur in every statistical week
    ...
    } # closes if(DO_GSI_ON_PTABLE==TRUE)
    ####################################################################################################
